...
register_base_types()
{
    VOID_TYPE = new BasicType("void", "void",
            "XXX", "XXX", "XXX", "XXX", "XXX",
            "XXX", "XXX", "XXX", "XXX", "XXX");
    NAMES.Add(VOID_TYPE);
    ...
    STATUS_TYPE = new BasicType("status_t", "status_t",
                                "XXX", "XXX", "XXX", "XXX", "XXX",
                                "XXX", "XXX", "XXX", "XXX", "XXX");
    NAMES.Add(STATUS_TYPE);

    BYTE_TYPE = new BasicType("byte", "int8_t",
    ...
    INT_TYPE = new BasicType("int", "int32_t",
            "writeInt32", "readInt32", "writeIntArray", "createIntArray", "readIntArray",
            "putInteger", "getInteger", "putIntegerArray", "createIntegerArray", "getIntegerArray");
    NAMES.Add(INT_TYPE);

    UINT_TYPE = new BasicType("uint32_t", "uint32_t",
            "writeInt32", "readInt32", "writeIntArray", "createIntArray", "readIntArray",
            "putInteger", "getInteger", "putIntegerArray", "createIntegerArray", "getIntegerArray");
    NAMES.Add(UINT_TYPE);

    LONG_TYPE = new BasicType("long", "int64_t",
            "writeInt64", "readInt64", "writeLongArray", "createLongArray", "readLongArray",
            "putLong", "getLong", "putLongArray", "createLongArray", "getLongArray");
    NAMES.Add(LONG_TYPE);

    FLOAT_TYPE = new BasicType("float", "float",
    ...
    DOUBLE_TYPE = new BasicType("double", "double",
...
void Type::SetCPPQualifiedName() {
    m_cppQualifiedName.clear();
    m_cppQualifiedName += m_qualifiedName;
    replaceAll(m_cppQualifiedName, ".", "::");
    replaceAll(m_cppQualifiedName, m_name, m_cppName);
}
     ...
     m_canBeOut(canBeOut)
{
    ...
    m_qualifiedName += name;
    this->SetCPPQualifiedName();
}

void Type::SetCPPName(const string& cppName) {
    m_cppName = cppName;
    this->SetCPPQualifiedName();
}
...
BasicType::BasicType(const string& name, const string& qName, const string& marshallParcel,
...
{
    this->SetCPPName("bool");
    this->SetQualifiedName("bool");
}

void
BooleanType::WriteToParcel(StatementBlock* addTo, Variable* v, Variable* parcel, int flags)
{
    addTo->Add(new MethodCall(parcel, "writeInt32", 1,
...
BooleanType::CreateFromParcel(StatementBlock* addTo, Variable* v, Variable* parcel, Variable**)
{
    addTo->Add(new Assignment(v, new Comparison(new LiteralExpression("0"),
                    "!=", new MethodCall(parcel, "readInt32"))));
}
...
{
    addTo->Add(new Assignment(v, new MethodCall(parcel, "readInt32"), this));
}
...
StringType::StringType()
    :Type("android", "String", BUILT_IN, true, true, false)
{
    SetCPPName("String16");
    SetCPPQualifiedName();
    this->SetQualifiedName("java.lang.String");
}

string
StringType::CreatorName() const
{
    return "android.Parcel.STRING_CREATOR";
}

void
StringType::WriteToParcel(StatementBlock* addTo, Variable* v, Variable* parcel, int flags)
{
    addTo->Add(new MethodCall(parcel, "writeString16", 1, v));
}

void
StringType::CreateFromParcel(StatementBlock* addTo, Variable* v, Variable* parcel, Variable**)
{
    addTo->Add(new Assignment(v, new MethodCall(parcel, "readString16")));
}
...
CharSequenceType::WriteToParcel(StatementBlock* addTo, Variable* v, Variable* parcel, int flags)
{
    ...
    elsepart->statements->Add(new MethodCall(parcel, "writeInt32", 1,
                                new LiteralExpression("0")));
    IfStatement* ifpart = new IfStatement;
    ifpart->expression = new Comparison(v, "!=", NULL_VALUE);
    ifpart->elseif = elsepart;
    ifpart->statements->Add(new MethodCall(parcel, "writeInt32", 1,
...
IBinderType::IBinderType()
    :Type("android", "IBinder", BUILT_IN, true, false, false)
{
    SetCPPName("sp<android::IBinder>");
    SetCPPQualifiedName();
    this->SetQualifiedName("android.IBinder");
}
...
IInterfaceType::IInterfaceType()
    :Type("android", "IInterface", BUILT_IN, false, false, false)
{
}
...
ParcelType::ParcelType()
    :Type("android", "Parcel", BUILT_IN, false, false, false)
{
}
...
UserDataType::WriteToParcel(StatementBlock* addTo, Variable* v, Variable* parcel, int flags)
{
    ...
    ifpart->statements->Add(new MethodCall(v, "writeToParcel", 1,
                                parcel));
